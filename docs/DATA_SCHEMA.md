...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v4.1 | 2026-06-08 | 补充 stock_adj_factor、stock_daily_snapshot 表结构；更新分区方案说明（年度分区 vs 月度分区）；更新目录结构 |
...
| 表名 | 用途 | 记录数（预计） | 存储类型 |
|------|------|---------------|----------|
| `stock_basic` | 股票基本信息 | ~5,000 | 元数据 |
| `stock_quotes` | 行情数据（多周期统一存储） | ~150,000,000 | 热数据（年度分区表） |
| `stock_indicators` | 技术指标 | ~150,000,000 | 热数据（年度分区表） |
| `stock_adj_factor` | 复权因子 | ~30,000,000 | 热数据 |
| `trade_signals` | 交易信号（MACD金叉/死叉） | ~500,000 | 热数据 |
| `stock_daily_snapshot` | 每日行情快照（含技术指标/估值/资金流） | ~5,000,000 | 快照数据 |
...
| `adj_factor` | NUMERIC | NOT NULL | 复权因子 |
| `created_at` | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP | 创建时间 |

**索引**:
- `PRIMARY KEY (code, trade_date)`
- `idx_adj_factor_date` - 按日期查询复权因子

**数据来源**：Baostock `query_adjust_factor` 接口

**表注释**:
```sql
COMMENT ON TABLE stock_adj_factor IS '复权因子表';
COMMENT ON COLUMN stock_adj_factor.adj_factor IS '复权因子，用于前复权/后复权价格计算';
```

---

### 3.15 stock_daily_basic（每日基本面指标表）

**用途**: 存储每日基本面估值指标，来源为 Tushare Pro `daily_basic` 接口（优先级1）和 Baostock `query_history_k_data_plus`（优先级2）
...
| `pe` | NUMERIC(14,4) | | 市盈率（静） |
| `pe_ttm` | NUMERIC(14,4) | | 市盈率（TTM） |
| `pb` | NUMERIC(14,4) | | 市净率 |
| `total_mv` | NUMERIC(18,2) | | 总市值（万元） |
| `circ_mv` | NUMERIC(18,2) | | 流通市值（万元） |
| `dv_ratio` | NUMERIC(10,4) | | 股息率（静，%） |
| `dv_ttm` | NUMERIC(10,4) | | 股息率（TTM，%） |
| `ps` | NUMERIC(10,2) | | 市销率（静） |
| `ps_ttm` | NUMERIC(10,2) | | 市销率（TTM） |
| `float_share` | NUMERIC(18,4) | | 流通股本（万股） |
| `created_at` | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP | 创建时间 |

**同步逻辑**: `sync_daily_basic.py` 使用 `DataSourceManager` + FAILOVER 策略。Tushare `daily_basic` 接口限速 **5次/天**（Tushare Pro 官方限制），每天同步窗口有限；Baostock 作为备用数据源提供 pe/pb/turnover_rate 等基础指标

### 3.16 stock_daily_snapshot（每日行情快照表）

**用途**: 前端行情展示的核心宽表，整合行情、技术指标、估值、资金流向等数据，避免多表 JOIN
...
| `adjust_type` | VARCHAR(10) | DEFAULT 'qfq' | 复权类型 |
| `change` | NUMERIC | | 涨跌额 |
...
| `is_new` | BOOLEAN | DEFAULT false | 是否新股 |
| `limit_up` | BOOLEAN | DEFAULT false | 是否涨停 |
| `limit_down` | BOOLEAN | DEFAULT false | 是否跌停 |
| `area` | VARCHAR(50) | | 地区 |
| `volume_ratio` | NUMERIC | | 量比 |
| `net_mf_amount` | NUMERIC | | 净流入（元） |
| `pe_ttm` | NUMERIC | | 滚动市盈率 |
| `ps` | NUMERIC | | 市销率 |
| `ps_ttm` | NUMERIC | | 滚动市销率 |
| `dv_ratio` | NUMERIC | | 股息率 |
| `dv_ttm` | NUMERIC | | 滚动股息率 |
| `float_share` | NUMERIC | | 流通股 |
| `buy_sm_amount` | NUMERIC | | 小单买入 |
| `sell_sm_amount` | NUMERIC | | 小单卖出 |
| `buy_md_amount` | NUMERIC | | 中单买入 |
| `sell_md_amount` | NUMERIC | | 中单卖出 |
| `buy_lg_amount` | NUMERIC | | 大单买入 |
| `sell_lg_amount` | NUMERIC | | 大单卖出 |
| `buy_elg_amount` | NUMERIC | | 特大单买入 |
| `sell_elg_amount` | NUMERIC | | 特大单卖出 |
| `break_high_20` | BOOLEAN | | 是否20日新高 |
| `break_high_60` | BOOLEAN | | 是否60日新高 |
| `consec_up_days` | INTEGER | | 连涨天数 |
| `vol_ratio_5` | NUMERIC | | 5日量比 |
...
| 索引名 | 字段 | 用途 |
|--------|------|------|
| `idx_snapshot_date` | `trade_date` | 按日期查询快照 |
| `idx_snapshot_code_date` | `code, trade_date` | 按股票和日期查询 |

**表注释**:
```sql
COMMENT ON TABLE stock_daily_snapshot IS '每日行情快照宽表（整合行情/指标/估值/资金流）';
```
...
**stock_quotes** 和 **stock_indicators** 表使用 **年度分区**（按 `trade_datetime` 年份），非月度分区。

| 表名 | 分区格式 | 分区范围 | 分区个数 |
|------|----------|----------|----------|
| `stock_quotes` | `stock_quotes_YYYY` | 2000-2027 | 28 |
| `stock_indicators` | `stock_indicators_YYYY` | 2015-2027 | 13 |

> **注意**：`stock_quotes_minute` 分钟线表使用 **月度分区**（格式 `stock_quotes_minute_YYYYMM`）。

**分区维护命令**：
```sql
-- 添加新年度分区
CALL add_year_partition('stock_quotes', 2028);
CALL add_year_partition('stock_indicators', 2028);
...
│  └──────────────┘ └──────────────┘ └──────────────────┘        │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────────┐        │
│  │ stock_basic  │ │stock_adj_factor│ │  trade_calendar  │        │
│  │  (元数据)    │ │  (复权因子)   │ │  (元数据)        │        │
│  └──────────────┘ └──────────────┘ └──────────────────┘        │
│  ┌──────────────┐ ┌──────────────────┐                         │
│  │stock_daily_  │ │  sync_checkpoints │                         │
│  │ snapshot     │ │  (水位线)        │                         │
│  └──────────────┘ └──────────────────┘                         │
└─────────────────────────────────────────────────────────────────┘
                               │
