...
用法：
    python signal_precompute.py                          # 增量计算（默认截止到指标表最新日期）
    python signal_precompute.py --end-date 2026-06-30    # 指定截止日期
    python signal_precompute.py --force-full             # 全量重新计算（忽略增量过滤）
"""
import sys
import os
import json
                ...
                with storage.transaction() as conn:
                    with conn.cursor() as cur:
                        ...
                        if result and result[0]:
                            ...
                            logger.info(f"📅 从 stock_indicators 获取最新日期: {end_date}")
                        else:
                            # 若指标表为空，则默认昨天
                            end_date = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")
                            logger.warning(f"⚠️ stock_indicators 表无数据，回退到昨天: {end_date}")
            except Exception as e:
