#!/usr/bin/env python3
"""K 线形态 ETL: stock_quotes_2026 → stock_indicators_2026 (5 个 pattern_* 列).

K 2026-06-17 决策：
  - 真库: TA-Lib 0.6.8 (已在 .venv 装好)
  - 参数: penetration=0.3 (统一)
  - 输出值: -100/-80/0/80/100 (TA-Lib 原生, 含强度)
  - 范围: stock_indicators_2026 (K 决策先跑 2026, 验证 OK 再扩全历史)

用法:
  python backend/clean/etl/pattern_precompute.py              # 全 2026 市场
...
def process_stock(cur, code: str, year_table: str = 'stock_quotes_2026') -> int:
...
def main():
    ...
    if args.code:
        # 单只
        n = process_stock(cur, args.code, year_table)
        conn.commit()
        logger.info(f'✅ {args.code}: 更新 {n} 行')
        conn.close()
        return
    ...
    total = len(codes)
    logger.info(f'开始 {year_table} 全市场 K 线形态 ETL: {total} 只')
    ...
    for i, code in enumerate(codes):
        try:
            n = process_stock(cur, code, year_table)
            if n > 0:
                success += 1
            total_rows += n
            # 每 200 只 commit 一次
