#!/usr/bin/env python3
"""
calc_additional_indicators_fast.py - 快速计算额外技术指标

针对 stock_daily_snapshot 中现有的记录计算：
...
logger = setup_logger('calc_additional_indicators_fast')
...
def calc_indicators():
    ...
    try:
        # 获取 stock_daily_snapshot 中的日期范围
        cur = conn.cursor()
        cur.execute("SELECT MIN(trade_date), MAX(trade_date) FROM stock_daily_snapshot")
        min_date, max_date = cur.fetchone()
        cur.close()
        
        logger.info(f"📅 快照日期范围：{min_date} 到 {max_date}")
        
        # 获取需要计算的股票代码（只针对快照中有记录的股票）
        cur = conn.cursor()
        ...
        for idx, code in enumerate(codes):
            try:
                ...
                if len(rows) < 65:  # 需要至少65天数据才能计算60日新高和5日量比
                    continue
                ...
                # 只保留快照日期范围内的数据
                df = df[(df['trade_date'] >= pd.to_datetime(min_date)) & 
                        (df['trade_date'] <= pd.to_datetime(max_date))]
                ...
                if (idx + 1) % 200 == 0:
                    logger.info(f"  进度：{idx+1}/{len(codes)}, 已更新：{total_updated}")
