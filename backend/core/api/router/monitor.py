...
def get_data_summary():
    ...
    if latest_quote:
        ...
        missing_real = missing_in_quote - missing_delisted - missing_suspended - missing_merged
        ...
        # 添加警告：如果有效覆盖率异常低（< 95%），提示有真正缺失的数据
        if missing_real > 0:
            result["warnings"].append({
                "level": "warning",
                "type": "real_missing",
                "message": f"有 {missing_real} 只活跃股票缺少最新交易日数据，建议检查数据下载",
                "suggestion": "运行 Baostock 补下载脚本补充缺失数据"
            })
