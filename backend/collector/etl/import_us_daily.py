...
- stock_quotes：cycle='1d'，market='us'，成交价列 open/high/low/close 统一存【后复权价】
  （与指标/回测/前端统一用 adj_close 一致）；另存 raw_open..raw_close + adj_open..adj_close。
- 仅交易日日线：不占盘前盘后（Yahoo download_single prepost=False 已保证）。
- 无涨跌停：不涉及 limit_up/down 逻辑（与港股一致）。
- 复权因子/除权日写 stock_adj_factor（market='us'，factor_date 列），仅写入发生除权
  因子变化的因子日（避免以 ~1.0 的每日因子淹没该表）。
- 增量控制：读 etl_control 表 market='us' 的 last_sync_date；--incremental 从 last_sync_date+1
...
def get_last_sync_date(conn: psycopg2.extensions.connection) -> Optional[str]:
    """读取 etl_control 中 market='us' 的 last_sync_date；无记录返回 None。"""
    try:
...
def clean_and_split(df_raw: pd.DataFrame, code: str) -> Tuple[Optional[pd.DataFrame], Optional[pd.DataFrame]]:
    ...
    quotes = pd.DataFrame({
        ...
        'trade_datetime': (
            # 先转纯日期，再拼收盘时间并本地化为美股时区（规避 Yahoo 索引自带时区导致 tz_localize 报错）
...
def main() -> None:
    ...
    parser.add_argument('--test-one', type=str, default=None, help='拉取单只验证（如 AAPL）')
