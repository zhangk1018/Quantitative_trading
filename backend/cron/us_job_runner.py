#!/usr/bin/env python3
"""
美股 ETL 独立调度 Runner（协作单 30.0 V6，K 2026-09-03 调整口径）
...
用法：
    ./venv/bin/python backend/cron/us_job_runner.py
    USJOB_RETRY_INTERVAL=0 ./venv/bin/python backend/cron/us_job_runner.py  # 关重试间隔(测试)
"""
import os
import sys
import time
...
def _run_script(args: List[str], step: str) -> int:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if attempt < MAX_RETRIES:
            sec = int(os.environ.get('USJOB_RETRY_INTERVAL', str(RETRY_INTERVAL_DEFAULT)))
            if sec > 0:
    ...
    append_alerts([{
        'level': 'CRITICAL',
        'market': MARKET,
        'message': f"[美股] [{step}] 重试 {MAX_RETRIES} 次仍失败，等待次日覆盖（检查数据源/网络/alerts.log）",
    }])
    return last_code
...
def main() -> None:
    """主函数：美股全链路串行执行。"""
    logger.info("=" * 70)
    logger.info(f"🚀 [美股] ETL 全链路开始（{datetime.now()}）")
    failed: List[str] = []
    for label, args in STEPS:
        ...
        if code != 0:
            failed.append(label)
    if failed:
        logger.error(f"🛑 [美股] 完成，失败步骤（已重试并告警）: {failed}")
    else:
        logger.info(f"✅ [美股] ETL 全链路成功（列表→日线→基本面→指标→形态→信号→宽表→Parquet）")
    logger.info("=" * 70)
