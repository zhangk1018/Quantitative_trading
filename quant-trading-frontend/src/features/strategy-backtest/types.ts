// src/features/strategy-backtest/types.ts
// 策略回测模块类型定义 | 迁移自 features/backtest/strategyBacktestTypes.ts
 ...
 *
 * 职责分层契约（K审阅V3.1#2）：
 * - tree 参数仅承载选股逻辑（FilterNode AST），不包含任何策略执行参数
 * - 执行参数（资金/费用/风控/调仓频率）一律从 Storage 读取
 * - 若 tree 中意外包含执行参数，解析时主动剥离并告警
 */
...
export class IndicatorCache {
  ...
  getMACD(component: 'dif' | 'dea' | 'hist', idx: number): number | null {
    const key = `macd${component.charAt(0).toUpperCase() + component.slice(1)}`;
    if (!this.isReady(key, idx)) return null;
    switch (component) {
  ...
  getBOLL(band: 'upper' | 'mid' | 'lower', idx: number): number | null {
    const key = `boll${band.charAt(0).toUpperCase() + band.slice(1)}`;
    if (!this.isReady(key, idx)) return null;
    switch (band) {
